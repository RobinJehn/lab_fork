...
class Bezier:
    def __init__(
        ...
        if self.size_ < 1 or self.T_max_ <= self.T_min_:
            raise ValueError(
                "Can't create Bezier curve; min bound is higher than max bound."
            )
    ...
    def eval_horner(self, t: float) -> np.ndarray:
    ...
    def derivative(self, order: int) -> Bezier:
        if order == 0:
            return self
        ...
        deriv = Bezier(
            ...
            self.T_min_,
            self.T_max_,
            self.mult_T_ * (1.0 / (self.T_max_ - self.T_min_)),
        )
