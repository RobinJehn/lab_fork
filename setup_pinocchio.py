...
def loadrobot() -> RobotWrapper:
    return RobotWrapper.BuildFromURDF(NEXTAGE_URDF, MESH_DIR)


def translate(robotwrapper: RobotWrapper, oMf: pin.SE3):
...
def translaterobot(robotwrapper: RobotWrapper, oMf: pin.SE3):
    for i in range(2):
        robotwrapper.collision_model.geometryObjects[i].placement = (
            oMf * robotwrapper.collision_model.geometryObjects[i].placement
        )
        robotwrapper.visual_model.geometryObjects[i].placement = (
            oMf * robotwrapper.visual_model.geometryObjects[i].placement
        )
    robotwrapper.model.jointPlacements[1] = oMf * robotwrapper.model.jointPlacements[1]


def addcollisiontorobot(robot: RobotWrapper, objectenv: RobotWrapper):
    # change joint parent
...
def loadobject(
    parentrobot: RobotWrapper, urdf: str, mesh: str, oMf: pin.SE3
) -> RobotWrapper:
...
def finalisecollisionsetup(robot: RobotWrapper):
    robot.collision_data = robot.collision_model.createData()
...
def addcubecollision(cube: RobotWrapper, obstacle: RobotWrapper):
...
def finalisecubecollisionsetup(cube: RobotWrapper):
...
def setuppinocchio() -> Tuple[RobotWrapper, RobotWrapper, RobotWrapper, RobotWrapper]:
